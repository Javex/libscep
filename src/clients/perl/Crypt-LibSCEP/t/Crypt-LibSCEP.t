use strict;
use warnings;
our $sig_cert = "-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
";

our $sig_key = "-----BEGIN PRIVATE KEY-----

-----END PRIVATE KEY-----
";

our $req = "-----BEGIN CERTIFICATE REQUEST-----

SLYGjIEJ2RwX
-----END CERTIFICATE REQUEST-----
";

our $enc_cacert = "-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----";

our $sig_cacert = "-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
";

our $sig_cakey = "-----BEGIN RSA PRIVATE KEY-----

-----END RSA PRIVATE KEY-----
";

our $enc_cert = "-----BEGIN CERTIFICATE-----

n+Hm9q37a9O3404+jkNZjOwQtANC72KR5QtRtkhq
-----END CERTIFICATE-----
";

our $enc_key = "-----BEGIN PRIVATE KEY-----

iX5k7rlRcZzQ
-----END PRIVATE KEY-----";

our $enc_cakey = "-----BEGIN RSA PRIVATE KEY-----
Proc-Type: 4,ENCRYPTED
DEK-Info: DES-EDE3-CBC,DD73214CD86AA97E


-----END RSA PRIVATE KEY-----
";

our $issuedCert = "-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
";

our $sig_cakey_enc = "-----BEGIN RSA PRIVATE KEY-----
Proc-Type: 4,ENCRYPTED
DEK-Info: DES-CBC,4F58E19A09903724


-----END RSA PRIVATE KEY-----
";

our $crl = "-----BEGIN X509 CRL-----

5fgZ7fdUlkoXpCUJbvudyGSzUtEas9dFQB7M8RGDGiBP
-----END X509 CRL-----
";

#my $conf = {log=>"/dev/null"};
my $conf = {};
use Test::More tests => 9;
BEGIN { use_ok('Crypt::LibSCEP') };

#Testing PKCSreq
my $pkcsreq = Crypt::LibSCEP::pkcsreq($conf, $sig_key, $sig_cert, $enc_cacert, $req);
ok($pkcsreq ne "", "pkcsreq creation");
my $pkcsreq_parsed = Crypt::LibSCEP::parse($conf, $pkcsreq);
ok($pkcsreq_parsed ne "", "pkcsreq parsed");
ok(Crypt::LibSCEP::get_transaction_id($pkcsreq_parsed) eq "2F3C88114C283E9A6CD57BB8266CE313DB0BEE0DAF769D770C4E5FFB9C4C1016", "message type");
ok(Crypt::LibSCEP::get_message_type($pkcsreq_parsed) eq "PKCSReq", "transID");
ok(Crypt::LibSCEP::get_signer_cert($pkcsreq_parsed) eq $sig_cert, "signerCert");
ok(Crypt::LibSCEP::get_senderNonce($pkcsreq_parsed) ne "", "senderNonce");
my $pkcsreq_unwrapped = Crypt::LibSCEP::unwrap({passin=>"pass", passwd=>"asdf"}, $pkcsreq, $sig_cacert, $enc_cacert, $enc_cakey);
ok($pkcsreq_unwrapped ne "", "unwrap");
ok(Crypt::LibSCEP::get_pkcs10($pkcsreq_unwrapped) eq $req, "get PKCS10");

#Testing Getcert
#my $getcert = Crypt::LibSCEP::getcert($conf, $sig_key, $sig_cert, $enc_cacert, $req);
#ok($getcert ne "", "pkcsreq creation");
#my $getcert_parsed = Crypt::LibSCEP::parse($conf, $getcert);
#ok($getcert_parsed ne "", "pkcsreq parsed");
#ok(Crypt::LibSCEP::get_transaction_id($getcert_parsed) eq "2F3C88114C283E9A6CD57BB8266CE313DB0BEE0DAF769D770C4E5FFB9C4C1016", "message type");
#ok(Crypt::LibSCEP::get_message_type($getcert_parsed) eq "PKCSReq", "transID");
#ok(Crypt::LibSCEP::get_signer_cert($getcert_parsed) eq $sig_cert, "signerCert");
#ok(Crypt::LibSCEP::get_senderNonce($getcert_parsed) ne "", "senderNonce");
#my $getcert_unwrapped = Crypt::LibSCEP::unwrap({passin=>"pass", passwd=>"asdf"}, $getcert, $sig_cacert, $enc_cacert, $enc_cakey);
#ok($getcert_unwrapped ne "", "unwrap");
#ok(Crypt::LibSCEP::get_pkcs10($pkcsreq_unwrapped) eq $req, "get PKCS10");


